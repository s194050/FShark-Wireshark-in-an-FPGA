...
# Verilog Ethernet MAC folder
VERIMACDIR?=$(CURDIR)/hardware/WiresharkMAC/fpga/SIM
# Crc32 calculator
CRCDIR?=$(CURDIR)/hardware/Crc32
...
emulator:
	...
	-cd $(HWBUILDDIR) && verilator -Wno-TIMESCALEMOD -I$(VERIMACDIR) --cc ../harnessConfig.vlt Patmos.v --top-module Patmos +define+TOP_TYPE=VPatmos --threads 1 -CFLAGS "-Wno-undefined-bool-conversion -O1 -DTOP_TYPE=VPatmos -DVL_USER_FINISH -include VPatmos.h"  -Mdir $(HWBUILDDIR) --exe ../Patmos-harness.cpp -LDFLAGS -lelf --trace
